...
deepstate_ci_setup<-function(path){
 ...
 write_to_tests <-paste0(write_to_tests,"result <- RcppDeepState::package_deepstate_pkg_create(@","package_path","@)\n")
 write_to_tests <-paste0(write_to_tests,"test_that(@deepstate create TestHarness@,{\nexpect_equal(result,@Testharness created!!@)\n})\n")
 write_to_tests <-paste0(write_to_tests,"RcppDeepState::package_deep_harness_compile_run(@","package_path","@)\n")
